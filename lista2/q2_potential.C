...
double update_V(vector<vector<double>> *V, vector<vector<bool>> *map) {
    ...
    for (int i = (0 + 1); i < (ROW - 1); i++) {
        for (int j = (0 + 1); j < (COL - 1); j++) {
            if ((*map)[i][j]) continue; //boundary condition
...
// if (i == 0) {
//     (*Ex)[i][j] = (-1.) * ((*V)[i + 1][j] - (*V)[i][j]);
//     continue;
// }
// if (j == 0) {
//     (*Ey)[i][j] = (-1.) * ((*V)[i][j + 1] - (*V)[i][j]);
//     continue;
// }
// if (i == ROW - 1) {
//     (*Ex)[i][j] = (-1.) * ((*V)[i][j] - (*V)[i - 1][j]);
//     continue;
// }
// if (j == COL - 1) {
//     (*Ey)[i][j] = (-1.) * ((*V)[i][j] - (*V)[i][j - 1]);
//     continue;
// }
...
void draw_E_field(vector<vector<double>> *Ex, vector<vector<double>> *Ey) {
    TFile output("q2/q2_eletricfield.root", "RECREATE");
    auto c = new TCanvas("c", "", 0, 0, 800, 600);
    c->Range(-1, -1, 20, 20);
    TGaxis *yaxis = new TGaxis(0, gPad->GetUymin(), 0, gPad->GetUymax(), -1, 20, 21, "");
    TGaxis *xaxis = new TGaxis(gPad->GetUxmin(), 0, gPad->GetUxmax(), 0, -1, 20, 21, "");
    vector<TArrow> ar;
    int count = 0;
    int ROW = Ex->size();
    ...
    for (int i = 0; i < ROW; i++) {
        for (int j = 0; j < COL; j++) {
            count++;
            double mod_E = 0.6; // * sqrt(POW2((*Ex)[i][j]) + POW2((*Ey)[i][j]));
            //cout << "mod E" << mod_E << endl;
            if ((*Ex)[i][j] == 0 && (*Ey)[i][j] == 0) continue;
            // ar.push_back(TArrow(i, j, i + ((*Ex)[i][j] / mod_E), j + ((*Ey)[i][j] / mod_E), 0.01));
...
void draw_potencial(vector<vector<double>> *V) {
    ...
    TFile output("q2/q2.root", "RECREATE");
    ...
    dt->Draw("surf1");
    dt->Write();
    c->SaveAs("q2/potential_field.png");
    c->Close();
    output.Close();
}
...
void laplace_calculate() {
    ...
    int iteration = 0;
    while (mean_convergence > 1e-6 || iteration < 50) {
        mean_convergence = update_V(&V, &boundary_map);
        iteration++;
    }
